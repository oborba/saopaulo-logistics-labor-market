...
def main():
    ...
    total_paid_activity = df[df['exerce_atividade_remunerada'] == 'S'][
        'qtd_condutores'
    ].sum()
    ...
    * **Grupo C:** Caminhões (Carga Média/Urbana).
    ...
    def define_logistics_group(cat):
        if 'E' in cat:
            return 'Grupo E (Articulados)'
        if 'D' in cat:
            return 'Grupo D (Passageiros)'
        if 'C' in cat:
            return 'Grupo C (Caminhão)'
        return 'Outros'

    df_heavy['grupo_logistico'] = df_heavy['categoria_cnh'].apply(
        define_logistics_group
    )

    # --- ROW 1: EXECUTIVE SUMMARY (CARDS) ---

    # KPI 1: Active Professionals (Heavy + EAR Yes)
    total_professionals = df_heavy[df_heavy['exerce_atividade_remunerada'] == 'S'][
        'qtd_condutores'
    ].sum()

    # KPI 2: Top of the Pyramid (Any category containing E)
    total_cat_e = df_heavy[df_heavy['grupo_logistico'] == 'Grupo E (Articulados)'][
        'qtd_condutores'
    ].sum()

    # KPI 3: Diversity (% Women in heavy fleet)
    total_heavy_general = df_heavy['qtd_condutores'].sum()
    total_women_heavy = df_heavy[df_heavy['genero'] == 'FEMININO'][
        'qtd_condutores'
    ].sum()
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric(
        'Motoristas de pesados com EAR', f'{total_professionals:,.0f}'.replace(',', '.')
    )
    ...
    with c_left:
        ...
        df_pie = (
            df_charts.groupby('grupo_logistico')['qtd_condutores'].sum().reset_index()
        )
    ...
    with c_right:
        ...
        df_bars = (
            df_charts.groupby(['grupo_logistico', 'exerce_atividade_remunerada'])[
                'qtd_condutores'
            ]
            .sum()
            .reset_index()
        ...
        if not df_bars.empty:
            df_bars['exerce_atividade_remunerada'] = df_bars[
                'exerce_atividade_remunerada'
            ].map({'S': 'Profissional (EAR)', 'N': 'Sem EAR'})
    ...
    map_filter = (df['exerce_atividade_remunerada'] == 'S') & (
    ...
    df_map = (
        df[map_filter]
        .groupby(['descricao_municipio', 'lat', 'lon'])['qtd_condutores']
        .sum()
        .reset_index()
    )
    ...
    for _, row in df_map.iterrows():
        folium.Circle(
            ...
            radius=row['qtd_condutores']
            * 0.05,  # Proportional radius (adjust multiplier as needed)
    ...
    st.info(
        '💡 **Análise:** Note como a mancha vermelha segue as principais rodovias (Anhanguera/Bandeirantes e Dutra).'
    )

    st.markdown(
        'Pesquise sua cidade abaixo para entender a vocação logística da sua região.'
    )
    ...
    def render_ranking(df_source, grupo_filtro, titulo, key_sulfix):
        ...
        df_rank = df_rank.sort_values(
            by='qtd_condutores', ascending=False
        ).reset_index()
        ...
        search_term = st.text_input(
            f'🔍 Buscar cidade ({titulo})', key=f'search_{key_sulfix}'
        )
    ...
    df_heavy_paid = df_heavy_total[
        df_heavy_total['exerce_atividade_remunerada'] == 'S'
    ].copy()
    ...
    count_young = df_heavy_paid[df_heavy_paid['faixa_etaria'].isin(young_age_ranges)][
        'qtd_condutores'
    ].sum()
    count_seniors = df_heavy_paid[
        df_heavy_paid['faixa_etaria'].isin(senior_age_ranges)
    ]['qtd_condutores'].sum()
    ...
    df_bar_chart = (
        df_heavy_total.groupby(['faixa_etaria', 'exerce_atividade_remunerada'])[
            'qtd_condutores'
        ]
        .sum()
        .reset_index()
    ...
    df_bar_chart['faixa_etaria'] = pd.Categorical(
        df_bar_chart['faixa_etaria'], categories=age_group_order, ordered=True
    )
    df_bar_chart = df_bar_chart.sort_values('faixa_etaria')

    # Rename Legend to be friendly
    df_bar_chart['exerce_atividade_remunerada'] = df_bar_chart[
        'exerce_atividade_remunerada'
    ].map({'S': 'Profissional Ativo', 'N': 'Reserva (Sem EAR)'})
    ...
    fig_wall = px.bar(
        ...
        color_discrete_map={
            'Profissional Ativo': '#EF553B',
            'Reserva (Sem EAR)': '#636EFA',
        },
        barmode='stack',
    )
    ...
    df_cliff_chart = (
        df_cliff.groupby(['faixa_etaria', 'grupo_comparativo'])['qtd_condutores']
        .sum()
        .reset_index()
    )
    ...
    df_cliff_chart['faixa_etaria'] = pd.Categorical(
        df_cliff_chart['faixa_etaria'], categories=age_group_order, ordered=True
    )
    ...
    fig_cliff = px.bar(
        ...
        labels={
            'qtd_condutores': 'Qtd. Motoristas Profissionais',
            'faixa_etaria': 'Idade',
        },
    ...
    * **Verde:** <10% da frota tem mais de 60 anos.
    ...
    ranges_60_plus = [
        ...
        '81-90 ANOS',
        '91-100 ANOS',
        'MAIOR DE 100 ANOS',  # Adjust according to your unique()
    ]

    # Base Grouping: Total HEAVY Drivers by city
    df_city_total = (
        df_map_target.groupby(['descricao_municipio', 'lat', 'lon'])['qtd_condutores']
        .sum()
        .reset_index()
    )
    ...
    df_seniors = (
        df_map_target[df_map_target['faixa_etaria'].isin(ranges_60_plus)]
        .groupby('descricao_municipio')['qtd_condutores']
        .sum()
        .reset_index()
    )
    ...
    df_scatter = pd.merge(
        df_city_total, df_seniors, on='descricao_municipio', how='left'
    )
    df_scatter['total_60_plus'] = df_scatter['total_60_plus'].fillna(0)

    # Risk Percentage Calculation
    # (Total Heavy Seniors / Total Heavy in City)
    df_scatter['pct_risco'] = (
        df_scatter['total_60_plus'] / df_scatter['total_pesados']
    ) * 100
    ...
    for _, row in df_scatter.iterrows():
        ...
        if row['total_pesados'] > 50:
            ...
            folium.CircleMarker(
                ...
                fill_color=colormap(
                    row['pct_risco']
                ),  # The color now reflects the aging OF THE SECTOR
                fill_opacity=0.8,
                popup=folium.Popup(
                    f"""
